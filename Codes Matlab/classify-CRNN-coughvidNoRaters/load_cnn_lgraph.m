function lgraph_cnn = load_cnn_lgraph(net_name, freeze_layers)
...
switch net_name
    ...
    case "yamnet"
        ...
        lgraph_cnn = layerGraph(net_cnn.Layers);
        ...
        lgraph_cnn = removeLayers(lgraph_cnn, layerNames);
...
%             string(net_cnn.Layers(22, 1).Name)
%             string(net_cnn.Layers(23, 1).Name)
...
%             ];
    ...
    case "openl3"
        ...
        % remove layers from CNN: 1st (input) and last (flatten, regression)
        layerNames = [
            string(net_cnn.Layers(1, 1).Name)
            string(net_cnn.Layers(29, 1).Name)
        ...
        lgraph_cnn = layerGraph();
        for i = 1:numel(layers)
            lgraph_cnn = addLayers(lgraph_cnn,layers(i));
        end
        
        % connect layers by the connections in |connections|
        for c = 1:size(connections,1)
            lgraph_cnn = connectLayers(lgraph_cnn,connections.Source{c},connections.Destination{c});
        end
end
