...
switch netName
    ...
    case "yamnet"
        ...
        lgraphCnn = layerGraph(netCnn.Layers);
        
        % remove layers from CNN: 1st (input) and last (fc, softmax, classification)
        layerNames = [
            string(netCnn.Layers(1, 1).Name) % image input layer
            string(netCnn.Layers(84, 1).Name) % fc
            string(netCnn.Layers(85, 1).Name) % softmax
            string(netCnn.Layers(86, 1).Name) % classification layer (loss function)
            ];
    ...
    case "vggish"
        ...
        layerNames = [
            string(netCnn.Layers(1, 1).Name)
            string(netCnn.Layers(24, 1).Name)
            ];
        lgraphCnn = removeLayers(lgraphCnn, layerNames);
    ...
    case "openl3"
        ...
        layerNames = [
            string(netCnn.Layers(1, 1).Name)
            string(netCnn.Layers(30, 1).Name)
            ];
        lgraphCnn = removeLayers(lgraphCnn, layerNames);
