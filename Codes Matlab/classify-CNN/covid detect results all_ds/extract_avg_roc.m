...
load pred_prior_roc_100_iters_shuffle_all_ds_tpr_0.6.mat

% pre-allocation
num_thresholds = 1000;
num_iters = size(lbls_pred_vs_true, 1);
...
for set_indx = [val_indx train_indx test_indx]
    ...
    for iter = 1 : num_iters
        ...
        lbls_pred = lbls_pred_vs_true{iter, pred_indx}{set_indx};
        lbls_true = lbls_pred_vs_true{iter, true_indx}{set_indx};
...
for set_indx = [val_indx train_indx test_indx]
    ...
    for iter = 1 : num_iters
        ...
        % get pred and true labels from cell
        lbls_pred = (lbls_pred_vs_true{iter, pred_indx}{set_indx} >= roc_threshold);
        lbls_true = string(lbls_pred_vs_true{iter, true_indx}{set_indx});
    ...
    scores(:, :, set_indx) = [acc, uar, F1, sensitivity, PPV, specificity];
end
...
scores_avg = reshape(scores_avg, 6, 3)';

scores_avg = array2table( ...
    scores_avg, ...
    'VariableNames', {'accuracy [%]' 'uar [%]' 'f1_score [%]' 'sensitivity [%]' 'ppv [%]' 'specificity [%]'}, ...
